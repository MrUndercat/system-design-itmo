...
export function registerRoutes(app: Express): void {
  ...
  app.get("/listings/:id", async (req: Request, res: Response) => {
    ...
    const { rows: photos } = await pool.query("SELECT id, photo_id FROM listing_photos WHERE listing_id = $1", [
      req.params.id,
    ]);
    res.json({
      ...rows[0],
      photos: photos.map((photo) => ({
        ...
        photoId: (photo as { photo_id: string }).photo_id,
        url: buildPhotoUrl((photo as { photo_id: string }).photo_id),
      })),
    });
  });
  ...
  app.post("/listings/:id/photos", requireAuth, async (req: AuthRequest, res: Response) => {
    ...
    if (!photoId) return res.status(400).json({ message: "photoId required" });
    ...
    const { rows } = await pool.query(
      `INSERT INTO listing_photos (listing_id, photo_id) VALUES ($1,$2) RETURNING id, listing_id, photo_id`,
      [req.params.id, photoId]
    );
    const created = rows[0] as { id: string; listing_id: string; photo_id: string };
    res.status(201).json({
      id: created.id,
      listingId: created.listing_id,
      photoId: created.photo_id,
      url: buildPhotoUrl(created.photo_id),
    });
  });
