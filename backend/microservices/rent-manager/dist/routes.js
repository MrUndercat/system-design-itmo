...
function registerRoutes(app) {
    ...
    app.get("/listings/:id", async (req, res) => {
        ...
        const { rows: photos } = await db_1.pool.query("SELECT id, photo_id FROM listing_photos WHERE listing_id = $1", [
            req.params.id,
        ]);
        res.json({
            ...rows[0],
            photos: photos.map((photo) => ({
                ...
                url: (0, s3_1.buildPhotoUrl)(photo.photo_id),
            })),
        });
    });
    app.post("/listings/:id/photos", require_auth_1.requireAuth, async (req, res) => {
        ...
        if (!photoId)
            return res.status(400).json({ message: "photoId required" });
        ...
        const { rows } = await db_1.pool.query(`INSERT INTO listing_photos (listing_id, photo_id) VALUES ($1,$2) RETURNING id, listing_id, photo_id`, [req.params.id, photoId]);
        const created = rows[0];
        res.status(201).json({
            id: created.id,
            listingId: created.listing_id,
            photoId: created.photo_id,
            url: (0, s3_1.buildPhotoUrl)(created.photo_id),
        });
    });
